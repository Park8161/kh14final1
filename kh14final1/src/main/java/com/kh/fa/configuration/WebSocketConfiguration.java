...
// - FE가 접속할 수 있는 주소를 설정
// - STOMP 방식의 구조가 작동할 수 있도록 메세지 브로커(=도우미)를 활성화
...
public class WebSocketConfiguration implements WebSocketMessageBrokerConfigurer {
	...
	public void configureMessageBroker(MessageBrokerRegistry registry) {
		// 구독 가능한 채널들을 설정
		registry.enableSimpleBroker("/public", "/private"); // 전송
		// 메세지를 전송할 수 있는 목적지의 접두사(prefix) 설정
		registry.setApplicationDestinationPrefixes("/app"); // 수신
	}
	
	// [2] 웹소켓 연결 자체에 대한 설정
	@Override
	public void registerStompEndpoints(StompEndpointRegistry registry) {
		registry.addEndpoint("/ws") // 웹소켓 연결을 위한 주소 설정 (주소의 마지막 / 뒤의 주소) 
